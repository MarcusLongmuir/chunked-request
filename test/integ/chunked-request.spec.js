...
describe('chunked-request', () => {
  ...
  it('should parse a response that consists of a single chunk', done => {
    ...
    const onHeaders = (headers, status) => {
      ...
      expect(isEqual(headers.getAll("my-header"), ["My-Header-Value"])).toBe(true, 'received headers');
  ...
  describe('response object', () => {
    ...
    it('500 Internal Server Error', done => {
      ...
      const onHeaders = (headers, status) => {
        ...
        expect(isEqual(headers.getAll("my-error-header"), ["My-Error-Header-Value"])).toBe(true, 'received headers');
        onHeadersCalled = true;
      };
